...
esriRequest, esriLang, LayerInfoFactory) {
  return declare(LayerInfo, {
    ...
    setSubLayerVisible: function(layersVisible) {
      ...
      //var visibleLayers = this._getVisibleLayers(this.originOperLayer.layerObject);
      //ary = [-1, -1, -1].concat(visibleLayers);
      //this.originOperLayer.layerObject.setVisibleLayers(ary);
    },
/////////
    _getVisibleLayers: function (a, c) {
      var b = a.layerInfos, d, e = [-1];
      "undefined" !== typeof c && (b[c].defaultVisibility = !b[c].defaultVisibility);
      for (d = 0; d < b.length; d++) {
        var f = b[d];
        f.defaultVisibility && (e.push(f.id), f = array.lastIndexOf(e, -1), -1 !== f && e.splice(f, 1))
      }
      b = [];
      for (d = 0; d < e.length; d++)f = e[d], this._allIdsPresent(a, f, e) && b.push(f);
      d = [];
      for (e = 0; e < b.length; e++)(f = this._getLayerInfo(a,
          b[e])) && null === f.subLayerIds && d.push(b[e]);
      d.length || (d = [-1]);
      return d
    },
    _allIdsPresent: function (a, c, b) {
      a = this._walkUpLayerIds(a, c);
      return array.every(a, function (a) {
        return -1 < array.indexOf(b, a)
      })
    },
